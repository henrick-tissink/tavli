  ...
  return (
    <div className="space-y-10">
      <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard label="Locații active" value={activeCount} icon={Building2} />
        <StatCard label="Plan" value={tier === "pro" ? "Tavli Pro" : "Tavli"} icon={BadgeCheck} tone="muted" />
        <StatCard label="Rezervări azi" value={today?.bookings ?? 0} />
        <StatCard label="Acoperiri azi" value={today?.covers ?? 0} />
      </section>

      <section>
        <div className="flex items-center justify-between">
          <h2 className="font-display text-xl text-text-primary">Locațiile tale</h2>
          <Link
            ...
            .map((v) => (
              ...
              >
                ...
                <p className="mt-1 inline-flex items-center gap-1 text-sm text-text-secondary">
                  <MapPin size={13} aria-hidden /> {v.cityName ?? "—"}
                </p>
        ...
        {tier === "pro" && activeCount > 3 && (
          <p className="mt-4 text-xs text-text-muted">
            {activeCount - 3} locații suplimentare × €15/lună peste cele 3 incluse în Pro.
          </p>
        )}
      </section>
    </div>
