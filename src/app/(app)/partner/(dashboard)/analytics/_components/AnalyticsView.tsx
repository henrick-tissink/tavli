...
function delta(n: number): string {
  return n > 0 ? `▲ +${n} față de săpt. trecută` : n < 0 ? `▼ ${n} față de săpt. trecută` : "— neschimbat";
}
    ...
    <div className="px-4 py-6 desktop:px-8 desktop:py-8">
      ...
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-border pb-6">
        ...
        <Button variant="secondary" onClick={() => setExportOpen(true)}>
          <span className="flex items-center gap-2">
            <Download size={16} /> Exportă datele
          </span>
        </Button>
      </header>
      ...
      {empty ? (
        <div className="rounded-card border border-dashed border-border bg-surface-bg/60 p-12 text-center">
          <h2 className="font-display text-2xl font-bold text-text-primary">Încă nu avem date</h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-text-secondary leading-relaxed">
            După primele rezervări, aici vei vedea acoperiri, neprezentări, mixul de grup și sursele de
            trafic — actualizate în fiecare noapte.
          </p>
        </div>
      ) : (
        <>
          ...
          <section className="mb-8 grid grid-cols-2 gap-4 desktop:grid-cols-4">
            <StatCard label="Rezervări" value={data.overview.bookings} icon={CalendarCheck} hint={delta(data.overview.bookingsDelta)} />
            <StatCard label="Acoperiri" value={data.overview.covers} icon={Users} hint={delta(data.overview.coversDelta)} />
            <StatCard label="Finalizate" value={data.overview.completed} icon={CheckCircle2} tone="success" />
            <StatCard label="Neprezentări" value={data.overview.noShows} icon={XCircle} tone={data.overview.noShows > 0 ? "warning" : "muted"} />
          </section>

          <div className="grid grid-cols-1 gap-5 desktop:grid-cols-2">
            {/* Covers per service */}
            <ChartCard kicker="Ultimele 30 de zile" title="Acoperiri pe serviciu">
              {data.coversPerService.length === 0 ? (
                <EmptyChart message="Acoperirile pe serviciu apar după primele rezervări finalizate." />
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={data.coversPerService} margin={{ left: -16 }}>
                    ...
                    <Bar dataKey="covers" name="Acoperiri" fill={ACCENT} radius={[6, 6, 0, 0]} maxBarSize={56} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </ChartCard>
            ...
            {/* No-show trend */}
            <ChartCard kicker="Ultimele 90 de zile" title="Rata de neprezentare">
              {data.noShowTrend.length === 0 ? (
                <EmptyChart message="Tendința neprezentărilor apare pe măsură ce se acumulează rezervări." />
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={data.noShowTrend} margin={{ left: -16 }}>
                    ...
                    <Line type="monotone" dataKey="rate" name="Rată" stroke={ACCENT} strokeWidth={2.5} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </ChartCard>
            ...
            {/* Party size mix */}
            <ChartCard kicker="Ultimele 90 de zile" title="Mixul de grup">
              {data.partyMix.every((p) => p.count === 0) ? (
                <EmptyChart message="Distribuția mărimii grupurilor apare după primele rezervări." />
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={data.partyMix} margin={{ left: -16 }}>
                    ...
                    <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: AXIS }} />
                    <Tooltip content={<ChartTooltip />} cursor={{ fill: "#F9731611" }} />
                    <Bar dataKey="count" name="Rezervări" radius={[6, 6, 0, 0]} maxBarSize={56}>
            ...
            <ChartCard kicker="Ultimele 90 de zile" title="Motive de anulare">
              {data.cancellations.length === 0 ? (
                <EmptyChart message="Nicio anulare în această perioadă — excelent." />
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
          ...
          <div className="mt-10">
            <div className="mb-5 flex items-center gap-2">
              ...
              <h2 className="font-display text-2xl font-bold text-text-primary">Panouri Pro</h2>
            </div>
            {isPro ? (
              <ProPanels data={data} />
            ) : (
              <div className="rounded-card border border-border bg-gradient-to-br from-brand-primary-soft to-surface-white p-8">
                <h3 className="font-display text-xl font-bold text-text-primary">Analize avansate</h3>
                <p className="mt-2 max-w-lg text-sm text-text-secondary leading-relaxed">
                  Hartă de căldură a neprezentărilor, retenția cohortelor, distribuția timpului de
                  rezervare, atribuirea surselor și prognoza pe 4 săptămâni — disponibile cu planul Pro.
                </p>
                <Button className="mt-4">Treci la Pro</Button>
              </div>
            )}
          </div>
        </>
...
function ProPanels({ data }: { data: AnalyticsViewData }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 desktop:grid-cols-2">
      {/* Heat map */}
      <ChartCard kicker="Ultimele 90 de zile" title="Hartă neprezentări" span="full">
        {!heatHasData ? (
          <EmptyChart message="Harta de căldură apare după 30 de zile de rezervări." />
        ) : (
            ...
            <div className="min-w-[640px]">
              ...
              {data.heatMap.map((row, dow) => (
                <div key={dow} className="flex items-center">
                  <div className="w-10 text-xs font-semibold text-text-secondary">{DOW_RO[dow]}</div>
                  {row.map((rate, h) => (
                    <div
                      key={h}
                      title={rate == null ? "fără date" : `${DOW_RO[dow]} ${h}:00 — ${Math.round(rate * 100)}% neprezentări`}
      ...
      {/* Cohort retention */}
      <ChartCard kicker="Lună de lună" title="Retenția cohortelor">
        {data.cohort.length === 0 ? (
          <EmptyChart message="Retenția cohortelor apare pe măsură ce clienții revin." />
        ) : (
      ...
      {/* Lead time */}
      <ChartCard kicker="Ultimele 90 de zile" title="Timp până la rezervare (mediană)">
        {data.leadTime.length === 0 ? (
          <EmptyChart message="Distribuția timpului de rezervare apare după primele rezervări." />
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={data.leadTime} margin={{ left: -8 }}>
              ...
              <Tooltip content={<ChartTooltip />} />
              <Line type="monotone" dataKey="p50" name="Median (min)" stroke={ACCENT} strokeWidth={2.5} dot={false} />
              <Line type="monotone" dataKey="p90" name="P90 (min)" stroke="#C2410C" strokeWidth={1.5} strokeDasharray="4 3" dot={false} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </ChartCard>
      ...
      {/* Channel attribution */}
      <ChartCard kicker="Ultimele 90 de zile" title="Atribuire surse">
        {data.channel.every((c) => c.count === 0) ? (
          <EmptyChart message="Atribuirea surselor apare după primele rezervări cu client identificat." />
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart layout="vertical" data={data.channel} margin={{ left: 24 }}>
              ...
              <Tooltip content={<ChartTooltip />} cursor={{ fill: "#F9731611" }} />
              <Bar dataKey="count" name="Rezervări" radius={[0, 6, 6, 0]} maxBarSize={22}>
      ...
      {/* Forecast */}
      <ChartCard kicker="Următoarele 28 de zile" title="Prognoză acoperiri" span="full">
        {data.forecast.length === 0 ? (
          <EmptyChart message="Prognoza apare după 12 săptămâni de rezervări pe fiecare zi a săptămânii." />
        ) : (
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={data.forecast} margin={{ left: -16 }}>
              ...
              <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: AXIS }} />
              <Tooltip content={<ChartTooltip />} cursor={{ fill: "#F9731611" }} />
              <Bar dataKey="predicted" name="Prognoză" fill={ACCENT} radius={[5, 5, 0, 0]} maxBarSize={20} />
              <Bar dataKey="confirmed" name="Deja confirmate" fill="#9A3412" radius={[5, 5, 0, 0]} maxBarSize={20} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </ChartCard>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-separate border-spacing-1 text-center text-xs">
        <thead>
          <tr>
            <th className="text-left font-semibold text-text-secondary">Cohortă</th>
