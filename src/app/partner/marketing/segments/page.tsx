  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-12">
      <header>
        ...
        <h1 className="mt-2 font-display text-4xl text-text-primary">Segmente</h1>
        <p className="mt-2 text-sm text-text-secondary">
          Construiește un public din baza ta de oaspeți și salvează-l pentru campanii.
        </p>
      </header>
            ...
            {segments.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-4 px-5 py-3.5 text-sm">
                ...
                <span className="text-text-muted">
                  {s.estimatedSize != null ? `≈ ${s.estimatedSize} oaspeți` : "—"}
                </span>
              </li>
            ))}
          </ul>
