...
function rowToForm(row: TableRow): FormState {
  return {
    label: row.label,
    description: row.description ?? "",
    capacityMin: String(row.capacityMin),
    capacityMax: String(row.capacityMax),
    capacityTypical: row.capacityTypical != null ? String(row.capacityTypical) : "",
    shape: row.shape,
    sectionId: row.sectionId ?? "",
    positionX: String(row.positionX),
    positionY: String(row.positionY),
    width: String(row.width),
    height: String(row.height),
    rotationDegrees: String(row.rotationDegrees),
    isBookableOnline: row.isBookableOnline,
    isProOnly: row.isProOnly,
  };
}

export function EditTableModal({ table, sections, onClose }: Props) {
  const router = useRouter();
  const [pending, start] = useTransition();
  const [form, setForm] = useState<FormState>(rowToForm(table));
    ...
    start(async () => {
      const res = await updateTableAction({
        ...
        changes: {
          ...
          capacityTypical: form.capacityTypical
            ? parseInt(form.capacityTypical, 10)
            : undefined,
          shape: form.shape as TableRow["shape"],
      ...
      <div className="relative w-full max-w-lg bg-surface-white rounded-card border border-border shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-surface-white z-10">
          <h2 className="font-display text-xl font-bold text-text-primary">
            Editează masa <span className="text-brand-primary">{table.label}</span>
          </h2>
          <button
            type="button"
            onClick={onClose}
