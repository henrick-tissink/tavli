/**
 * Partner /security server-action tests (§01 §5a.2 phase 2 sub-unit A).
 *
 * Mirror of the admin /security action tests; the partner versions thread
 * the `"venue_owner"` actorRole through the mfa.ts helpers and redirect
 * to /partner/sign-in instead of /admin/sign-in.
 */
...
const ROLE = "venue_owner" as const;
...
describe("regenerateRecoveryCodes", () => {
  it("calls generateRecoveryCodes with userId + role and returns codes", async () => {
    ...
    expect(r).toEqual({ ok: true, data: { codes: ["x-x-x", "y-y-y"] } });
...
describe("changePasswordAction", () => {
  ...
  it("calls changePassword with role on happy path then redirects", async () => {
    ...
    expect(redirect).toHaveBeenCalledWith(
      "/partner/sign-in?password_changed=1",
    );
  });
...
describe("signOutEverywhereAction", () => {
  it("calls signOutEverywhere with role then redirects", async () => {
    ...
    expect(redirect).toHaveBeenCalledWith("/partner/sign-in?signed_out=1");
  });
});
