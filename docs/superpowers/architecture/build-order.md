...
- [ ] §01 MFA / passkeys (§01 §5.2) *(phase 1 shipped 2026-05-22 — `src/lib/auth/mfa.ts` helper layer + audit + 7 tests; backend wrappers for enrol/verify/unenrol/list. Phase 2 (UI + admin sign-in enforcement) is a frontend-design follow-up unit that ships the /admin/security + /partner/security pages plus the AAL1→AAL2 challenge gate for admins. Passkeys/WebAuthn deferred to v1.5.)*
- [ ] §01 Tavli-admin support impersonation (§01 §5.3) *(phase 1 shipped 2026-05-22 — `src/lib/auth/impersonation.ts` audit primitives + 3 tests. Phase 2 (UI + session-cookie mechanism + persistent red banner + admin-side user-list trigger + AAL2 gate) is a frontend-design follow-up paired with MFA phase 2.)*
