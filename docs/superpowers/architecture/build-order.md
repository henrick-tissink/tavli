...
- [x] §12 Stripe webhook router with two-layer idempotency (§12 §6.3.1) *(shipped 2026-05-24 — Wave 5 sub-unit D; `/api/webhooks/stripe` via foundations ingestWebhook (layer-1) + `stripe-webhook-router.ts` per-event mirror handlers (subscription.*/invoice.*/payment_method.*/setup_intent.succeeded→attach card+send RecurringChargeConsentEmail+PSD2 audit/dispute/refund) + `webhook-idempotency.ts` layer-2 billing_audit_log dedup. Shared `stripe-status.ts` mapper. No keys (mocked events). Dunning emails/lockout on payment_failed deferred to W5-G.)*
